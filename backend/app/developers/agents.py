...
def _system(agent_type: str) -> str:
    ...
    backend_rule = (
        ...
        "the app at startup. Only import packages that REALLY EXIST on PyPI — never invent "
        "a package name. For RATE LIMITING use `slowapi` (`from slowapi import Limiter`); "
        "there is NO `starlette_limiter` package — importing it fails the whole install."
