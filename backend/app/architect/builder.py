...
# Brand-colour keyword -> (primary, primary-hover, accent). Only ever compared after
# lower-casing the BA-captured `design.brand_color`. Default is a warm coffee brown — a
# safe, friendly ground for the common food/local-business builds, overridden the moment
# the owner names a colour.
_BRAND_PALETTES = {
    "brown": ("#6f4e37", "#5a3d29", "#c8a27a"),
    "coffee": ("#6f4e37", "#5a3d29", "#c8a27a"),
    "tan": ("#a9744f", "#8c5e3f", "#d8b28c"),
    "orange": ("#ea580c", "#c2410c", "#fdba74"),
    "amber": ("#d97706", "#b45309", "#fcd34d"),
    "yellow": ("#d97706", "#b45309", "#fcd34d"),
    "red": ("#dc2626", "#b91c1c", "#fca5a5"),
    "rose": ("#e11d48", "#be123c", "#fda4af"),
    "pink": ("#db2777", "#be185d", "#f9a8d4"),
    "purple": ("#7c3aed", "#6d28d9", "#c4b5fd"),
    "violet": ("#7c3aed", "#6d28d9", "#c4b5fd"),
    "indigo": ("#4f46e5", "#4338ca", "#a5b4fc"),
    "blue": ("#2563eb", "#1d4ed8", "#93c5fd"),
    "teal": ("#0d9488", "#0f766e", "#5eead4"),
    "cyan": ("#0891b2", "#0e7490", "#67e8f9"),
    "green": ("#16a34a", "#15803d", "#86efac"),
    "emerald": ("#059669", "#047857", "#6ee7b7"),
    "black": ("#000000", "#1a1a1a", "#9ca3af"),
    "gray": ("#4b5563", "#374151", "#9ca3af"),
    "grey": ("#4b5563", "#374151", "#9ca3af"),
    "slate": ("#475569", "#334155", "#94a3b8"),
}
_DEFAULT_PALETTE = _BRAND_PALETTES["brown"]


def _brand_palette(summary: dict) -> tuple[str, str, str, bool]:
    """(primary, hover, accent, warm) from the BA-captured brand colour + vibe.

    Deterministic, forgiving: scans the free-text colour for a known keyword (so
    'warm brown', 'a nice deep blue' both resolve), and treats a warm colour OR a warm
    vibe as a cue for a cream ground instead of a cool grey one. Unknown -> coffee brown."""
    design = summary.get("design") or {}
    color = str(design.get("brand_color") or "").lower()
    vibe = str(design.get("style_vibe") or "").lower()
    primary, hover, accent = _DEFAULT_PALETTE
    matched = ""
    for kw, pal in _BRAND_PALETTES.items():
        if kw in color:
            primary, hover, accent = pal
            matched = kw
            break
    warm_words = ("brown", "coffee", "tan", "orange", "amber", "yellow", "red", "rose")
    warm = (matched in warm_words) or ("warm" in vibe) or ("warm" in color) or (matched == "")
    return primary, hover, accent, warm


def _design_system_css(summary: dict) -> str:
    """A polished, DETERMINISTIC design-system globals.css themed by the owner's captured
    brand colour + vibe. Replaces the old 'keep it minimal, plain CSS' directive that made
    every generated app look bare (run 1843 feedback: 'plain website, no animations'). Plain
    CSS only (no @tailwind/preprocessor -> `next build` can never fail on it). Styles native
    elements the generated pages actually render (button/input/select/textarea/a/h1-h6/table)
    so the whole app lifts even where pages use inline styles, plus tasteful motion behind a
    prefers-reduced-motion guard."""
    primary, hover, accent, warm = _brand_palette(summary)
    if warm:
        bg, surface, text, muted, border = "#faf6f1", "#ffffff", "#2b2320", "#6b5d52", "#ece3d8"
    else:
        bg, surface, text, muted, border = "#f6f7f9", "#ffffff", "#111827", "#6b7280", "#e5e7eb"
    return f"""/* Design system — auto-generated by the Architect from the owner's brand colour + vibe.
   Plain CSS only (no preprocessor). Edit the tokens in :root to re-theme the whole app. */
:root {{
  --color-bg: {bg};
  ...
  --shadow-lg: 0 14px 40px rgba(17, 24, 39, 0.12);
...
/* A soft warm wash at the top so the page never reads as a flat white sheet. */
body::before {{
  content: "";
  position: fixed;
  inset: 0 0 auto 0;
  height: 320px;
  background: radial-gradient(1200px 320px at 50% -120px,
...
/* Buttons — themed and animated. Inline styles on a page still win where set; everything
   else (radius, transition, hover lift, focus ring) is inherited from here. */
...
/* Card / surface helper the pages can opt into, plus a light lift on hover. */
.card, .surface {{
  background: var(--color-surface);
  border: 1px solid var(--color-border);
...
th, td {{ text-align: left; padding: 0.65rem 0.8rem; border-bottom: 1px solid var(--color-border); }}
th {{ color: var(--color-muted); font-weight: 600; font-size: 0.9rem; }}
tr {{ transition: background 0.12s var(--ease); }}
tbody tr:hover {{ background: color-mix(in srgb, var(--color-primary) 5%, transparent); }}
...
/* Respect users who prefer less motion — disable all animation/transition. */
...
def _frontend_globals_ticket(summary: dict | None = None) -> dict:
    ...
    Its `content` is now BAKED deterministically (a themed design system — run 1843
    'plain website' feedback), so it is written verbatim, never LLM-generated: reliable
    look on every build AND one fewer paid ticket. See `_design_system_css`.

    FND-* so it lands in the first wave; no dependencies.
    """
    return {
        "id": "FND-5",
        "title": "Frontend global stylesheet (themed design system)",
        "assigned_to": "frontend",
        "filepath": "frontend/app/globals.css",
        "is_boilerplate": True,
        "content": _design_system_css(summary or {}),
        "description": (
            "Deterministic themed design system for frontend/app/globals.css — plain CSS "
            "with brand-coloured tokens, styled native elements, and tasteful motion. "
            "Written verbatim from the ticket content; no generation needed."
        ),
        "dependencies": [],
    }
...
def _entrypoint_ticket(other_ids: list[str]) -> dict:
    ...
    return {
        ...
        "description": (
            ...
            "origin list read from the ALLOWED_ORIGINS environment variable (that "
            "EXACT name, comma-separated) — NEVER allow_origins=[\"*\"] together "
        ...
        tickets += [
            {
                ...
                "description": (
                    ...
                    "FALL BACK to Anthropic's vision model that reads the page "
                    "image(s) directly. Use the official Anthropic Python SDK "
                    "EXACTLY like this: `from anthropic import Anthropic` (the "
                    "client class is named `Anthropic` — there is NO `Claude` class, "
                    "never import one), then `client = "
                    "Anthropic(api_key=os.environ['MENU_EXTRACTION_API_KEY'])`, and "
                    "call `client.messages.create(model=<a current Claude vision "
                    "model>, max_tokens=..., messages=[{'role': 'user', 'content': "
                    "[<the page image as a base64 image block>, <the text prompt>]}])`. "
                    "Use a STRUCTURED prompt returning each item's dish name, price, "
                    "category and description as JSON. CRITICAL — the extraction and "
                    "parsing MUST be REAL working logic that turns an actual menu into "
                    "actual menu_items rows: the function that parses the extracted "
                    "text (or the model's reply) into items MUST genuinely parse and "
                    "RETURN them — it must NEVER be a placeholder/stub that returns an "
                    "empty list (a menu that has items MUST yield items; an empty "
                    "result may ONLY mean the menu truly had none). Read the model's "
                    "reply correctly: with the Anthropic SDK the text is "
                    "`response.content[0].text` (there is no `response['choices']`), "
                    "and pass an image as a proper base64 image block "
                    "`{'type': 'image', 'source': {'type': 'base64', 'media_type': "
                    "<the image's type>, 'data': <base64>}}`, never a bare string. "
